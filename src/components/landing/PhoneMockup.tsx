const timerLabels = ["Anos", "Meses", "Dias"] as const;
const timerValues = ["02", "14", "08"] as const;
      ...
      <div className="relative rounded-[2.75rem] border border-[var(--border)] bg-zinc-950 p-3 shadow-2xl premium-glow">
        ...
        <div className="relative min-h-[480px] aspect-[9/16] overflow-hidden rounded-[2.25rem] bg-black">
          <span
            ...
            aria-hidden
          />
          <span className="absolute inset-0 block bg-gradient-to-t from-black via-black/20 to-black/40" aria-hidden />
          ...
          <div className="relative z-10 flex h-full min-h-[480px] flex-col px-5 pt-14 pb-10">
            ...
            <p className="mt-1 text-center text-xs text-[var(--accent)]">Juntos há 842 dias</p>
            ...
            <ul className="mt-auto grid list-none grid-cols-3 gap-2 p-0">
              {timerValues.map((v, i) => (
                ...
                >
                  <span className="block text-xl font-semibold tabular-nums text-white">{v}</span>
